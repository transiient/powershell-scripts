...
Write-Output "Export AD Group Contact members"
...
$GroupName=Read-Host "Enter the name of the AD Group from which to export contacts"
...
Get-ADObject -Filter 'objectclass -eq "contact" -and memberof -eq $memberOf' -properties * | select name,mail,@{e={"$($_.memberOf)"};l="Member Of"} | Export-Csv "$CSVFileName" -NoTypeInformation
